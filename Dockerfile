...
COPY ./src /opt/src
COPY ./connectors-jars /opt/connectors-jars
RUN cd /opt; gradle clean build --no-daemon

FROM apache/flink:1.14.4-scala_2.12-java8

COPY --from=builder /opt/connectors-jars /opt/flink/lib
COPY --from=builder /opt/build/libs /opt/flink/lib
